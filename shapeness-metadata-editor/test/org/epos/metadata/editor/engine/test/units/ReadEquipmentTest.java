...
import org.epos.metadata.editor.engine.test.beans.Equipment;
 ...
 *
 */
public class ReadEquipmentTest {

	private static List<Class<?>> list;
	static Logger log = Logger.getLogger(ReadEquipmentTest.class.getName());
	 ...
	 * 
	 * 
	 * TEST ON EPOS-DCAT-AP -> Equipment
	 * 
	 * 
	 */

	@Test
	public void EquipmentFieldNumber() {
		for(Class<?> c : list) {
			if(c.getName().contains("Equipment")) {
				assertEquals(Equipment.class.getDeclaredFields().length,c.getDeclaredFields().length);
			}
		}
	}
	...
	@Test
	public void EquipmentFieldName() throws NoSuchFieldException, SecurityException {
		for(Class<?> c : list) {
			if(c.getName().contains("Equipment")) {
				for(Field f : c.getDeclaredFields()) {
					assertNotNull(Equipment.class.getDeclaredField(f.getName()));
				}
			}
		}
	}
	...
	@Test
	public void EquipmentFieldType() throws NoSuchFieldException, SecurityException  {
		for(Class<?> c : list) {
			if(c.getName().contains("Equipment")) {
				for(Field f : c.getDeclaredFields()) {
					log.info("Field : "+f.getName());
					log.info("Field type: "+f.getType().getSimpleName());
					log.info("Field type expected:  "+Equipment.class.getDeclaredField(f.getName()).getType().getSimpleName());
					assertEquals(Equipment.class.getDeclaredField(f.getName()).getType().getSimpleName(), f.getType().getSimpleName());
				}
			}
		}
	}
	...
	@Test
	public void EquipmentFieldAnnotation() throws NoSuchFieldException, SecurityException {
		for(Class<?> c : list) {
			if(c.getName().contains("Equipment")) {
				for(Field f : c.getDeclaredFields()) {
					assertEquals(Equipment.class.getDeclaredField(f.getName()).getDeclaredAnnotations().length, f.getDeclaredAnnotations().length);
					for(Annotation n : f.getDeclaredAnnotations()) {
						log.info("Annotation: "+n);
						log.info("Annotation Expected: "+Arrays.asList(Equipment.class.getDeclaredField(f.getName()).getDeclaredAnnotations()).toString());
						assertTrue(Arrays.asList(Equipment.class.getDeclaredField(f.getName()).getDeclaredAnnotations()).contains(n));
					}
	...
	@Test
	public void EquipmentFieldGettersName() throws NoSuchMethodException, SecurityException {
		for(Class<?> c : list) {
			if(c.getName().contains("Equipment")) {
				for(Method f : c.getDeclaredMethods()) {
					if(f.getName().startsWith("get"))
						assertEquals(Equipment.class.getDeclaredMethod(f.getName()).getName(), f.getName());
				}
			}
		}
	}
	...
	@Test
	public void EquipmentFieldGettersType() throws NoSuchMethodException, SecurityException {
		for(Class<?> c : list) {
			if(c.getName().contains("Equipment")) {
				for(Method f : c.getDeclaredMethods()) {
					if(f.getName().startsWith("get"))
						assertEquals(Equipment.class.getDeclaredMethod(f.getName()).getDeclaringClass().getSimpleName(), f.getDeclaringClass().getSimpleName());
				}
			}
		}
	}
	...
	@Test
	public void EquipmentFieldSettersName()throws NoSuchMethodException, SecurityException {
		for(Class<?> c : list) {
			if(c.getName().contains("Equipment")) {
				for(Method f : c.getDeclaredMethods()) {
					if(f.getName().startsWith("set"))
						assertTrue(Arrays.asList(Equipment.class.getDeclaredMethods()).stream().map(Method::getName).collect(Collectors.toList()).contains(f.getName()));
				}
			}
		}
	}
	...
	@Test
	public void EquipmentFieldAddersType() throws NoSuchMethodException, SecurityException {
		for(Class<?> c : list) {
			if(c.getName().contains("Equipment")) {
				for(Method f : c.getDeclaredMethods()) {
					if(f.getName().startsWith("add")) {
						List<Class<?>> list = Arrays.asList(Equipment.class.getDeclaredMethods()).stream()
	...
	@Test
	public void EquipmentFieldAddersName() throws NoSuchMethodException, SecurityException {
		for(Class<?> c : list) {
			if(c.getName().contains("Equipment")) {
				for(Method f : c.getDeclaredMethods()) {
					if(f.getName().startsWith("add")) {
	...
	@Test
	public void EquipmentFieldSettersType() throws NoSuchMethodException, SecurityException {
		for(Class<?> c : list) {
			if(c.getName().contains("Equipment")) {
				for(Method f : c.getDeclaredMethods()) {
					if(f.getName().startsWith("set")) {
	...
	@Test
	public void EquipmentMethodsParametersType() throws NoSuchMethodException, SecurityException {
		for(Class<?> c : list) {
			if(c.getName().contains("Equipment")) {
				...
				Arrays.asList(Equipment.class.getDeclaredMethods()).forEach(e->{
