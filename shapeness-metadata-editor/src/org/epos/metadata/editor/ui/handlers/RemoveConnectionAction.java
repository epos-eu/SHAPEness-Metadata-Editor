...
public class RemoveConnectionAction extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			...
			if(!selection.isEmpty()) {
				...
				////System.out.println("REMOVE CONNECTION: " + objSelected);
				if(objSelected instanceof EntityConnectionData) {
					EntityConnectionData selectedConnection = (EntityConnectionData) objSelected;
					Connection removedConnection = DataManager.getIstance().getConnectionByEntityConnectionData(selectedConnection);
					
					if(MessageDialog.openConfirm(HandlerUtil.getActiveShell(event), 
							"Question", "Do you really want to remove the relationship " + removedConnection.getLabel() + "?")) {
						
						DataManager.getIstance().removeConnection(removedConnection);
						
						Node source = removedConnection.getSource();
						Node destination = removedConnection.getDestination();
						String fieldName = removedConnection.getLabel();
						updateBeanRemoveObj(source, fieldName, destination, destination.getClass().getTypeName());
